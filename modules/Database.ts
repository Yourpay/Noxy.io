...
const pool_cluster: mysql.PoolCluster = mysql.createPoolCluster();
...
class DatabasePool implements iDatabasePool {
  ...
  constructor(id: string, config: DatabaseMasterEnvironmental) {
    ...
    store[id] = this;
    pool_cluster.add(id, this.__configuration);
  ...
  public query<T>(sql: string, replacers?: any | any[], options: iDatabaseQueryConfig = {}): Promise<T[] | T[][]> {
    ...
    return new Promise<T[] | T[][]>((resolve, reject) => {
      ...
      if (options.slave && options.master) { return pool_cluster.of(this.id).query(query, cb); }
      if (options.slave) { return pool_cluster.of(_.isString(options.slave) ? options.slave : (this.id + "::*")).query(query, cb); }
      return pool_cluster.of(this.id).query(query, cb);
    })
  ...
  private add(config: DatabaseEnvironmental) {
    ...
    pool_cluster.add(_.join([this.id, key], "::"), this.__slaves[key].__configuration);
  }
...
const exported: iDatabaseService = _.assign(
  Service,
  {
    ...
    get cluster() { return pool_cluster; },
    add:    add,
    update: update,
    remove: remove,
    parse:  parse
