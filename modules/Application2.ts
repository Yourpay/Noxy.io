...
import {eRequestMethods, iApplicationFn, iApplicationRouter, iApplicationService, Middleware, tApplicationParams, tApplicationRouters, tApplicationStatics, tApplicationSubdomains} from "../interfaces/iApplication";
import {tEnumValue} from "../interfaces/iAuxiliary";
import Route from "../resources/Route";

const Service: iApplicationFn = Default;
const __domain: string = env.domains[env.mode];
const __params: tApplicationParams = {};
const __statics: tApplicationStatics = {};
const __routers: tApplicationRouters = {};
const __subdomains: tApplicationSubdomains = {};
const __router: iApplicationRouter = {};
...
function addSubdomain(subdomain: string) {
  return __router[subdomain] = {}
}

function addStatic(public_directory_path: string, subdomain: string, namespace?: string): boolean {
  return !!(_.isString(namespace) && namespace !== "" ? _.set(__router, [subdomain, namespace, "static"], public_directory_path) : _.set(__router, [subdomain, "static"], public_directory_path));
}

function addParam(param: string, subdomain: string, mw: Middleware | Middleware[]): boolean
function addParam(param: string, subdomain: string, namespace: string, mw: Middleware | Middleware[]): boolean;
function addParam(param: string, subdomain: string, namespace: string | Middleware | Middleware[], mw?: Middleware | Middleware[]): boolean {
  if (!__router[subdomain]) { addSubdomain(subdomain); }
  
  if (mw) { return !!_.set(__router, [subdomain, <string>namespace, param], mw); }
  return !!_.set(__router, [subdomain, param], mw);
}

function addRoute(subdomain: string, namespace: string, path: string, method: tEnumValue<eRequestMethods>, mw: Middleware | Middleware[]): Route {
...
const exported: iApplicationService = _.assign(
  Service,
  {
    get router() { return __router; },
    get application() { return __application; }
  }
);
