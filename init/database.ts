...
database_queue.promise("create", (resolve, reject) => {
  ...
  Promise.map(_.values(Resource.list), resource =>
    new Promise((resolve, reject) => {
      if (databases[resource.table.options.resource.database]) { return databases[resource.table.options.resource.database].then(() => resolve()).catch(err => reject()); }
  ...
  .then(() => Promise.map(_.keys(databases), database => Database(database).query(`SET FOREIGN_KEY_CHECKS = 1;`)).then(() => resolve()))
  .catch(err => reject(err));
});
