...
db_queue.promise("create", (resolve, reject) => {
  Promise.map(Table.tables, (tables, database: string) => Database.namespace(database).query(`SET FOREIGN_KEY_CHECKS=0; ${_.join(_.map(tables, table => table.toSQL()), " ")} SET FOREIGN_KEY_CHECKS=1;`))
  .then(res => resolve(res))
  .catch(err => reject(err));
});
