...
export default class Role extends BaseObject {
  ...
  public static __fields = _.merge({}, BaseObject.__fields, {
    name: {type: "varchar(32)", required: true, onInsert: $this => console.log($this.name.toLowerCase().replace(/\s|\W/g, "")) || !($this.key = $this.name.toLowerCase().replace(/\s|\W/g, "")) || $this.name, onUpdate: $this => $this.__fields.name.onInsert($this)},
  ...
  }, BaseObject.generateTimeIndexes(), BaseObject.generateUserIndexes());
